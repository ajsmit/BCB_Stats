...
1. **Causality:** There should be a theoretical or philosophical basis for expecting a causal relationship, where the independent variable ($X$) influences or determines the dependent variable ($Y$).[^chapter_SLR-1] It is assumed that changes in $X$ cause changes in $Y$.
...
2. **Independence of Observations:**
    ...
    -   The observations of $Y$ must also be independently across the range of $X$ values. This means that the value of $Y$ at one point should not influence the value of $Y$ at another point.[^chapter_SLR-2]
...
4. **Dependent Variable:** Similarly, the dependent variable ($Y$) should also be measured on a continuous scale, such as integers, real numbers, intervals, or ratios.[^chapter_SLR-3] If your data are not continuous and the dependent variable is ordinal, use a *ordinal (logistic) regression*.

[^chapter_SLR-1]: The independent and dependent variables are also called the predictor and response variables, respectively. The predictor is often under the experimenter's control (in which case it is a fixed effects model), while the response is the variable predicted to respond in the manner hypothesised.

[^chapter_SLR-2]: If $Y$ not independent across the range of $X$, use a different type of regression model, such as a linear mixed-effects model.

[^chapter_SLR-3]: The dependent variable can also be ordinal, but this is less common. If this is the case, use *ordinal (logistic) regression instead.
...
where `formula` is a symbolic description  [using the notation by @wilkinson1973symbolic] of the model to be fitted, and `data` is the data frame containing the variables. The `...` argument is used to pass additional arguments to the function (consult `?lm`). For example: 
...
```{r, fig.width=c(6,5),fig.asp=0.65,out.width=c("60%","12cm")}
#| echo: true
...
```{r, fig.width=c(9,6),fig.asp=c(0.75,0.65),out.width=list("40%","12cm")}
#| echo: false
#| messages: false
#| warning: false
...
```{r, fig.width=c(6,5),fig.asp=0.85,out.width=c("60%","12cm")}
#| echo: true
